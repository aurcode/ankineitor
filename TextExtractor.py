...
class TextExtractor:
    ...
    def extract_text(self):
        if self.test:
            self.text = "这是我所喜欢的书。书。书。我所说的话你要记住。这个地方是游客所喜欢的"
            return
    ...
    def printDF(self, head=True, num = 10):
        if head:
    ...
    def separatedChineseCharacters(self, phrases=False):
        chinese_regex = r'[\u4E00-\u9FFF]+'
        ...
        self.df = df = pd.merge(df, df_aux, on='hanzi', how='left')[['hanzi','part_x','frequency']].drop_duplicates(subset='hanzi').rename(columns={'part_x':'part'}).sort_values(by='frequency', ascending=False).reset_index(drop=True)
